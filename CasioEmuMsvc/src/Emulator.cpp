...
namespace casioemu {
	...
	// ── ModelResourceStore forwarding ──────────────────────────────────────
	// These three were declared in Emulator.hpp (HasModelResource,
	// ReadModelResource, WriteModelSessionResource) but never had a body --
	// that's what threw "ModelResourceStore.h file not found" once the header
	// itself was added back, and would have thrown "undefined reference" at
	// link time right after even with the header present. When model_resources
	// is set (memory-backed model, e.g. resources packaged into an iOS/Android
	// app bundle instead of sitting at a filesystem path) reads and writes go
	// through the in-memory store. Otherwise it falls back to the same disk
	// path GetModelFilePath() already resolves, so desktop behavior is
	// unchanged.
	bool Emulator::HasModelResource(const std::string& name) const {
		if (model_resources)
			return model_resources->HasResource(name);
		std::ifstream probe(GetModelFilePath(name), std::ios::binary);
		return probe.good();
	}

	std::vector<std::uint8_t> Emulator::ReadModelResource(const std::string& name) const {
		if (model_resources)
			return model_resources->ReadResource(name);
		std::ifstream ifs(GetModelFilePath(name), std::ios::in | std::ios::binary);
		if (!ifs.good())
			return {};
	...
	void Emulator::WriteModelSessionResource(const std::string& name, const std::vector<std::uint8_t>& data) {
		if (model_resources) {
			model_resources->WriteSessionResource(name, data);
			return;
		}
		std::ofstream ofs(GetModelFilePath(name), std::ios::out | std::ios::binary | std::ios::trunc);
		if (ofs.good())
