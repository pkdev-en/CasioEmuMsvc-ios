...
int main(int argc, char* argv[]) {
        ...
        // bundle on its own, so relying on relative paths here — before
        // SDL_Init() establishes anything — silently resolves against the
        // wrong directory and leaves the app without any models, locales, or
        // fonts after the chdir() below runs.
  ...
  DiscordRPC::UpdatePresence("");
...
#ifdef __IOS__
        {
                ...
                // untouched). Calling this only works after SDL_Init(), hence
                // why this whole block lives here instead of at the top of main().
                ...
                if (home && !basePath.empty()) {
                        ...
                        // straight out of the (read-only) bundle directory.
...
#ifdef __IOS__
        // On iOS, the entire emulator session (model selection → emulator
        // creation → main event loop) lives inside this outer while(true)
        // so a Home Screen shortcut tapped while a model is already running
        // can swap to a different model without restarting the process.
        // See the matching `continue`/`break` near the end of main().
        while (true) {
#endif
                if (argv_map["model"].empty()) {
                        ...
                        // earlier, as part of the normal app-launch sequence).
#ifdef __IOS__
        ...
        std::thread t3([&]() {
                ...
                while (running) {
                        if (!busy)
                                SDL_PushEvent(&se);
#if defined(__ANDROID__) || defined(__IOS__)
                        // [Perf fix] Was SDL_Delay(40) — hard-capped every device at ~25fps
        ...
        while (emulator.Running()) {
                ...
                busy = false;
                DiscordRPC::Update();
                // [Perf fix] Was SDL_PollEvent() + continue — spins this thread at 100% of
...
#ifdef __IOS__
                {
                        // A Home Screen shortcut was tapped while a *different*
                        // model is already running — either a Quick Action
                        // (TakePendingShortcutModel(), set by CasioEmuAppDelegate.mm)
                        // or, for backward compatibility, a tap on a shortcut created
                        // by an older build of this app (the casioemu:// URL scheme,
                        // delivered as this SDL_DROPFILE event and resolved via
                        ...
                        // TakePendingShortcutModel() right after this loop ends and,
                        // if it returns something, loops back with the new model
                        // instead of exiting. See Ext/ShortcutLaunch.h.
        ...
        } // end scoped Emulator block (destructs emulator here)
#ifdef __IOS__
        {
