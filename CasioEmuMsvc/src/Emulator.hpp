...
namespace casioemu {
	...
	public:
		...
		// ── iOS / Android: các field cho CalculatorWindow và screenshot UI ────
		// tx: texture mà emulator render calculator lên (thay vì thẳng ra SDL window).
		// Chỉ được set khi build cho iOS/Android. Trên các platform khác luôn là nullptr.
		SDL_Texture* tx = nullptr;

		// screenshot_full_ui / screenshot_taken: dùng bởi Ui_ios.cpp
		std::atomic<bool> screenshot_full_ui{true};
		std::atomic<bool> screenshot_taken{false};

		// recording_full_ui: dùng bởi Ui_ios.cpp
		std::atomic<bool> recording_full_ui{true};

		// calculator_as_tab: khi true, CalculatorWindow render calc trong ImGui
		// (chỉ có ý nghĩa trên iOS/Android, desktop luôn false)
